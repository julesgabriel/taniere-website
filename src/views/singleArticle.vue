    ...
    <div class="article">
      ...
      <div class="content">
        <div class="top">
          ...
          <div class="right">
            <img v-for="(item, index) in article.authors" :key="index"
                 :src="envApiUrl + item.avatar.formats.thumbnail.url" alt="">
          </div>
        </div>
        ...
        <div class="authors">
          Article publié le
          {{ renderDateToMonthAndDateLitterals(article.date) }}<br>{{ renderAuthorsSentence(this.article.authors) }}
        </div>
        <div class="annexes">
          <p>Annexes</p>
          <div class="images">
...
export default {
  ...
  methods: {
    ...
    renderAuthorsSentence(authors) {
      ...
      if (authors.length > 1) {
        ...
        for (let i = 1; i < authors.length; i++) arrayToConcatSentenceWith.push(authors[i].name)
  ...
  mounted() {
    GetDataFetchedFromApi("articles", this.$route.params.slug)
